...
void AC_CustomControl::init(void)
{
    switch (CustomControlType(_controller_type))
    {
        ...
        case CustomControlType::CONT_PID:
            ...
            break;
        case CustomControlType::CONT_MC:
            _backend = NEW_NOTHROW AC_CustomControl_MC(*this, _ahrs, _att_control, _motors, _dt);
            _backend_var_info[get_type()] = AC_CustomControl_MC::var_info;
            //printf("\n\nHere!\n\n");
            break;
        default:
            return;
...
void AC_CustomControl::set_custom_controller(bool enabled)
{
    ...
    if (!enabled) {
        GCS_SEND_TEXT(MAV_SEVERITY_INFO, "TDC Controller is deactivated! (v2.0)");
        //printf("TDC Controller is deactivated!\n");
    ...
    if (enabled && _controller_type > CustomControlType::CONT_NONE) {
        ...
        _backend->reset();
        AP_Param::setup_object_defaults(this, var_info);
        GCS_SEND_TEXT(MAV_SEVERITY_INFO, "TDC Controller is activated! (v2.0)");
        //printf("TDC Controller is activated!\n");
    }
